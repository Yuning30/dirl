...
class TaskSpec:
    ...
    def __str__(self):
        if self.cons == Cons.ev:
            ...
            return f"{str(self.subtasks[0])} OR {str(self.subtasks[1])}"
        elif self.cons == Cons.repeat:
            return f"repeat({str(self.subtasks[0])})"
        else:
            assert False
    ...
    def quantitative_semantics_fast(self, traj, sys_dim, use_prefix=False):
        ...
        if use_prefix:
            return self.quantitative_semantics_dp_fast(sys_traj, res_traj)[1][
                len(traj) - 1
            ]
        else:
        ...
        if self.cons == Cons.ev:
            for i in range(n):
                if i == 0:
                    ...
                    retval[0][n - i - 1] = max(
                        retval[0][n - i],
                        self.predicate(sys_traj[n - i - 1], res_traj[n - i - 1]),
                    )
                    retval[1][i] = max(
                        retval[1][i - 1], self.predicate(sys_traj[i], res_traj[i])
                    )
            return retval
                ...
                for i in range(n):
                    if i == 0:
                        ...
                        retval[0][n - i - 1] = max(
                            retval[0][n - i],
                            min(
                                self.subtasks[0].predicate(
                                    sys_traj[n - i - 1], res_traj[n - i - 1]
                                ),
                                subval[0][n - i],
                            ),
                        )
                        subval[0][n - i - 1] = max(
                            subval[0][n - i], subval[0][n - i - 1]
                        )
                ...
                for i in range(n):
                    if i == 0:
                        ...
                        retval[1][i] = max(
                            retval[1][i - 1],
                            min(
                                self.subtasks[1].predicate(sys_traj[i], res_traj[i]),
                                subval[1][i - 1],
                            ),
                        )
            ...
            for i in range(n):
                for j in range(i, n):
                    if j == i:
                        ...
                        retval[i][j] = max(
                            retval[i][j - 1], self.predicate(sys_traj[j], res_traj[j])
                        )
            return retval
        ...
        if self.cons == Cons.seq:
            ...
            for i in range(n):
                for j in range(i, n):
                    ...
                    for k in range(i, j):
                        retval[i][j] = max(
                            retval[i][j], min(subval1[i][k], subval2[k + 1][j])
                        )
            return retval
    ...
    def get_monitor(self, state_dim, res_dim, local_reward_bound):
        ...
        if self.cons == Cons.ev:
            ...
            e00 = (0, cutil.true_predicate(local_reward_bound), cutil.id_update)
            e01 = (1, mpred, pred_update)
            e11 = (1, cutil.true_predicate(local_reward_bound), cutil.id_update)
            transitions = [[e00, e01], [e11]]
            rewards = [None, frew]

            return Monitor_Automaton(
                2, 1, state_dim + res_dim, np.array([0.0]), transitions, rewards
            )
        ...
        # adding safety constraints
            ...
            for ve in mon.transitions:
                ...
                for q, p, u in ve:
                    ve_new.append(
                        (
                            q,
                            cutil.project_predicate(p, 0, mon.n_registers),
            ...
            return Monitor_Automaton(
                n_states, n_registers, input_dim, init_reg, transitions, rewards
            )
            ...
            for qu in range(mon1.n_states):
                ...
                for qv, p, u in ve:
                    ve_new.append(
                        (
                            qv,
                            cutil.project_predicate(p, 0, mon1.n_registers),
            ...
            for ve in mon2.transitions:
                ...
                for q, p, u in ve:
                    ve_new.append(
                        (
                            q + mon1.n_states,
                            cutil.project_predicate(p, 0, mon2.n_registers),
        ...
        # choice
            ...
            for q1, p1, u1 in mon1.transitions[0]:
                if q1 == 0:
                    ...
                    trans_init.append(
                        (
                            q1,
                            cutil.project_predicate(p1, 0, mon1.n_registers),
            ...
            for q2, p2, u2 in mon2.transitions[0]:
                if q2 == 0:
                    ...
                    trans_init.append(
                        (
                            q2 + mon1.n_states - 1,
                            cutil.project_predicate(p2, mon1.n_registers, n_registers),
                            cutil.project_update(
                                u2, mon1.n_registers, n_registers, clean=True
                            ),
                        )
                    )
            ...
            def loop_update(state, reg):
                return np.concatenate(
                    [
                        us[0](state, reg[0 : mon1.n_registers]),
                        us[1](state, reg[mon1.n_registers : n_registers]),
                    ]
                )

            trans_init[0] = (0, cutil.true_predicate(local_reward_bound), loop_update)
            ...
            for ve in mon2.transitions[1:]:
                ...
                for q, p, u in ve:
                    ve_new.append(
                        (
                            q + mon1.n_states - 1,
                            cutil.project_predicate(p, mon1.n_registers, n_registers),
            ...
            for rew in mon1.rewards[1:]:
                if rew is not None:
                    rewards.append(cutil.project_reward(rew, 0, mon1.n_registers))
                else:
                    rewards.append(None)
            ...
            for rew in mon2.rewards[1:]:
                if rew is not None:
                    rewards.append(
                        cutil.project_reward(rew, mon1.n_registers, n_registers)
                    )
                else:
                    rewards.append(None)
        ...
        # conditional
            ...
            for q, p, u in mon2.transitions[0]:
                trans_init.append(
                    (
                        q + mon1.n_states + 1,
                        cutil.conj_pred(p, notb, mon2.init_registers),
            ...
            for rew in mon2.rewards:
                if rew is None:
                    ...
                    rewards.append(cutil.project_reward(rew, 0, mon2.n_registers))
...
def repeat(phi):
    """
    Repeat sequence task
    phi : TaskSpec
    """
    return TaskSpec(Cons.repeat, None, [phi])
