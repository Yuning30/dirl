...
class LinearProgNetwork(nn.Module):
    ...
    def forward(self, x):
        return 2 * F.sigmoid(self.program(x)) - 1
                ...
                torch.add(
                    torch.multiply(
                        ...
                        torch.multiply(
                            torch.multiply(torch.add(sc2, -1.0), -1.0),
                            torch.multiply(
                                torch.multiply(torch.add(sc, -1.0), -1.0), self.elsep21
                            ),
                        ),
                    ),
                    torch.multiply(
                        ...
                        torch.multiply(
                            torch.multiply(torch.add(sc2, -1.0), -1.0),
                            torch.multiply(
                                torch.multiply(torch.add(sc, -1.0), -1.0), self.elsep22
                            ),
                        ),
                    ),
                ),
...
class PendulumPIDProgNetwork(nn.Module):
    def __init__(self, obs_dim, act_dim):
        ...
        # PID parameters
        self.kp = nn.Linear(
            int(self.obs_dim / 5), self.act_dim, bias=False
...
class ProgPolicy:
    def __init__(self, observation_dim, action_dim, prog, min_log_std=-3, init_log_std=0, seed=None):
        """
        ...
        # Old Policy network
        # ------------------------
        self.old_model = copy.deepcopy(
            prog
        )  # FCNetwork(self.n, self.m, hidden_sizes=(), bias=bias)
        self.old_log_std = Variable(torch.ones(self.m) * init_log_std)
        self.old_params = list(
            filter(lambda p: p.requires_grad, self.old_model.parameters())
        ) + [self.old_log_std]
