...
class GymEnv(object):
    def __init__(self, env_name, art_test_env=False):
        if art_test_env:
            self.env = env_name
            self.env_id = "art_test_env"
            self._horizon = env_name.horizon
            self._action_dim = env_name.da
            self._observation_dim = env_name.ds
            ...
            try:
                self._elapsed_steps = self.env.env._elapsed_steps
            except AttributeError:
    ...
    def step(self, action):
        action = action if self.continuous_env else int(action)
